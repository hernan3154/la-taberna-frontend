...
async function cargarBebidasSinAlcohol() {
    try {
        const response = await fetch('https://api-rest-amber-tau.vercel.app/sinAlcohol');
        //const productos = await response.json();
        const bebidasinalcohol = await response.json();

        // Llamar a la función que crea las tarjetas de productos con los datos obtenidos de la API
        crearTarjetasBebidaSinAlcohol(bebidasinalcohol);
    } catch (error) {
...
function crearTarjetasBebidaSinAlcohol(bebidasinalcohol) {
    contenedorTarjetas.innerHTML = ""; // Limpiar el contenedor antes de agregar nuevas tarjetas

    bebidasinalcohol.forEach(producto => {
